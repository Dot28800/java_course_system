...
public class StudentController {
    ...
    /*@PostMapping("/try")
    public DataResponse getStudent(@Valid @RequestBody DataRequest dataRequest) {
        return new DataResponse(dataRequest.getNum(),null,null);
    ...
    public DataResponse studentDelete(@Valid @RequestBody DataRequest dataRequest) {
        ...
        if (s != null) {
            ...
            statisticsController.localStudentDelete(s.getPerson().getNum());
            leaveController.localStudentDelete(studentId);
            honorController.localDeleteStudent(studentId);
            scienceController.localStudentDelete(studentId);
            rankController.localStudentDelete(s.getStudentId());
            familyMemberController.localStudentDelete(s.getStudentId());
            consumptionController.localStudentDelete(studentId);
            practiceController.localStudentDelete(studentId);
            competitionController.localStudentDelete(s.getStudentId());
            markController.localStudentDelete(s.getStudentId());
            homeWorkController.localStudentDelete(studentId);
            honorController.localHonorDelete(studentId);
            courseChooseController.localStudentDelete(studentId);
            preController.localStudentDelete(studentId);
            courseAttendanceController.localDeleteStudent(studentId);
            scoreController.localDeleteStudent(studentId);
     ...
     */
    @PostMapping("/studentEditSave")//增以及改
    //@PreAuthorize(" hasRole('ADMIN')")
    public DataResponse studentEditSave(@Valid @RequestBody DataRequest dataRequest) {
        String num =dataRequest.getString("num");  //Map 获取属性的值
        Student s = null;
        Person p;
        User u;
        Optional<Student> op1=studentRepository.findByPersonNum(num);
        if (!op1.isPresent()) {
            return CommonMethod.getReturnMessageError("相应的序号不存在，请勿修改原来序号");
        }
        ...
        p.setName(dataRequest.getString("name"));
        p.setDept(dataRequest.getString("dept"));
        p.setCard(dataRequest.getString( "card"));
        p.setAge(dataRequest.getString("age"));
        p.setGender(gender);
        p.setBirthday(dataRequest.getString("birthday"));
        p.setEmail(dataRequest.getString( "email"));
        p.setPhone(dataRequest.getString("phone"));
        p.setAddress(dataRequest.getString("address"));
        personRepository.save(p);  // 修改保存人员信息
        s.setMajor(dataRequest.getString("major"));
        s.setClassName(dataRequest.getString("className"));
        s.setPerson(p);
        studentRepository.save(s);  //修改保存学生信息
        //systemService.modifyLog(s,isNew);
        return CommonMethod.getReturnData(s.getStudentId());

    }

    @PostMapping("/studentNewSave")
    public DataResponse studentNewSave(@Valid@RequestBody DataRequest dataRequest){
        //Integer studentId = dataRequest.getInteger("studentId");
        ...
        Optional<Student> op1=studentRepository.findByPersonNum(num);
        if (op1.isPresent()) {
            return CommonMethod.getReturnMessageError("相应的序号已经存在，请勿重复添加");
        }
        ...
        String genderName=dataRequest.getString("gender");
        String gender;
        if(genderName.equals("1")){
            gender="男";
        }else if(genderName.equals("2")){
            gender="女";
        }else{
            gender=genderName;
        }
