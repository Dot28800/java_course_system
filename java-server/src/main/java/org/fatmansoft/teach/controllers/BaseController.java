...
public class BaseController {
    ...
    public DataResponse editPassword(@Valid@RequestBody  DataRequest dataRequest){
        ...
        String newPassword=dataRequest.getString("password");
        Optional<User> op=userRepository.getByNumAndName(num,name);
        if(!op.isPresent()){
            return CommonMethod.getReturnMessageError("用户不存在！");
        }

        User u=op.get();
        if(u.getUserType().getId().equals(1)){
            return CommonMethod.getReturnMessageError("没有此权限");
        }
        u.setPassword(encoder.encode(newPassword));
        userRepository.save(u);
    ...
    @PostMapping("/getMenuList")//检查用户信息并返回标签集合给前端MainFrameController里面的初始化方法，datalist是由一系列map对象组成的列表
    public DataResponse getMenuList(@Valid @RequestBody DataRequest dataRequest) {
        ...
        }//userTypeId获取菜单权限种类（学生教师管理员），UserId 获取菜单实际编号
        List<MenuInfo> mList = menuInfoRepository.findByUserTypeIds(userTypeId + "");//根据不同的用户权限查找到不同的菜单记录
        Map m;
        List sList;
        for (MenuInfo info : mList) {//遍历可显示的所有菜单，将每一条记录的基本信息封装放入datalist中
            m = new HashMap();
            m.put("id", info.getId());//每次循环读取一条记录（一个菜单选项）里面的信息
    ...
    //@PreAuthorize("hasRole('ADMIN')")//管理员对应角色，只有管理员才能被接受请求，用于区分不同用户权限
