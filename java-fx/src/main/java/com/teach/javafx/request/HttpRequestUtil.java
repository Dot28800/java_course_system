...
public class HttpRequestUtil {
    ...
    //public static String serverUrl = "http://202.194.7.29:9090";
    //public static String serverUrl = "http://10.27.209.18:9090";
    ...
    public static String login(LoginRequest request){
        if(isLocal) {
            ...
            HttpRequest httpRequest = HttpRequest.newBuilder()//返回流对象
                    .uri(URI.create(serverUrl + "/auth/login"))
                    .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(request)))
                    .headers("Content-Type", "application/json")
                    .build();//返回httprequest对象
            try {
    ...
    public static DataResponse request(String url, DataRequest request){
        if(isLocal) {
            ...
            HttpRequest httpRequest = HttpRequest.newBuilder()
                    ...
                    .POST(HttpRequest.BodyPublishers.ofString(gson.toJson(request)))
                    .headers("Content-Type", "application/json")
                    .headers("Authorization", "Bearer " + AppStore.getJwt().getAccessToken())//把后台代表个人信息的加密串带过去
                    .build();
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    //                System.out.println(response.body())js
                    DataResponse dataResponse = gson.fromJson(response.body(), DataResponse.class);//反序化打印json
                    return dataResponse;
                }
            } catch (IOException e) {
                e.printStackTrace();
